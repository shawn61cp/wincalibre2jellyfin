...
"""wincalibre2jellyfin.py
   ...
   2023-11-17 initial revision, https://github.com/shawn61cp/wincalibre2jellyfin
...
class BookMetadata:
    ...
    def __init__(self, metadata_file_path: Path | None):
        ...
        self.read_metadata_file(metadata_file_path)
        if not self.doc:
            return
    ...
    def read_metadata_file_fallback(self, metadata_file_path: Path) -> None:

        """Reads book metadata file, sets sellf.doc, use in case of UnicodeDecodeError

            metadata_file_path      Path, full path to metadata file
    ...
    def read_metadata_file(self, metadata_file_path: Path) -> None:

        """Reads book metadata file, sets self.doc
...
class Book:
    ...
    def do_book(self) -> None:
        ...
        """Conditionally creates/updates destination book folder and file
        ...
        if self.book_file_dst_path.exists():
            if stat(self.book_file_dst_path).st_mtime < stat(self.book_file_src_path).st_mtime:
                copy_book = True
        else:
            copy_book = True
        ...
        if copy_book:
            try:
                copyfile(self.book_file_src_path, self.book_file_dst_path)
            except Exception as excep:
                logging.warning('Could not copy the book file %s: %s', self.book_file_dst_path, excep)
    ...
    def do_cover(self) -> None:
        ...
        if self.cover_file_src_path:
            ...
            if self.cover_file_dst_path.exists():
                if stat(self.cover_file_dst_path).st_mtime < stat(self.cover_file_src_path).st_mtime:
                    copy_cover = True
            else:
                copy_cover = True
            ...
            if copy_cover:
                try:
                    copyfile(self.cover_file_src_path, self.cover_file_dst_path)
                except Exception as excep:
                    logging.warning('Could not copy the cover image %s: %s', self.cover_file_dst_path, excep)
                    ...
                    if self.construct.foldermode in ['author,series,book', 'series,book']:
                        if self.metadata.titleel and self.construct.mangle_meta_title:
                            self.metadata.titleel.firstChild.data = (
                                f'{self.metadata.formatted_series_index} - {self.metadata.titleel.firstChild.data}'
                            )
